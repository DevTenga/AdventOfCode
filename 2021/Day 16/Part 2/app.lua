...
for _,file in ipairs(arg) do
	...
	--[[
		Byte Config:
			...
			4-6: Type ID
				Values:
				0 = Sum of all sub-packets.
				1 = Product of all sub-packets.
				2 = Smallest of all sub-packets.
				3 = Largest of all sub-packets.
				...
				5 = First subpacket greater than the second.
				6 = First subpacket lesser than the second.
				7 = First subpacket equal to the second.
	]]
	...
	function typeConfig.sum(packets)
		local sum = 0
	...
	function typeConfig.product(packets)
		local product = 1
		for _,packet in ipairs(packets) do
	...
	function typeConfig.min(packets)
		return math.min(table.unpack(packets))
	end

	function typeConfig.max(packets)
		return math.max(table.unpack(packets))
	end

	function typeConfig.greater(packets)
		return packets[1] > packets[2] and 1 or 0
	end

	function typeConfig.lesser(packets)
		return packets[1] < packets[2] and 1 or 0
	end

	function typeConfig.equal(packets)
		return packets[1] == packets[2] and 1 or 0
	end
	...
	function typeConfig:call(typeID,...)
		return self[self._config[typeID]](...)
	end
	...
	function parse_15_packet(binTable,nextIdx,typeID)
		...
		return typeConfig:call(typeID, packets), nextIdx
	end

	function parse_11_packet(binTable, nextIdx, typeID)
	...
	function parse_packet(binTable,nextIdx)
		...
		if typeID == 4 then 
			...
			if lenType == "0" then
				return parse_15_packet(binTable,nextIdx + 7, typeID)
			elseif lenType == "1" then
				return parse_11_packet(binTable,nextIdx + 7, typeID)
			end
	...
	-- =========================== Main ============================ --
	local answer = parse_packet(binData)
