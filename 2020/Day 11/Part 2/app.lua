	...
	for idx, row in ipairs(seating) do
		...
		for innerIdx, seat in ipairs(row) do
			...
			for _, pair in ipairs(selector) do
				local adjIdx, innerAdjIdx = idx, innerIdx 
				local adjSeat = '.'

				while adjSeat == '.' do
					if not seating[adjIdx + pair[1]] then
						break
					end
					adjIdx = adjIdx + pair[1]
					innerAdjIdx = innerAdjIdx + pair[2]

					adjSeat = seating[adjIdx][innerAdjIdx]
				end


				occCounter = adjSeat == "#" and occCounter + 1 or occCounter;
				empCounter = adjSeat == "L" and empCounter + 1 or empCounter;
				
				if seat == '#' and occCounter >= 5 then
